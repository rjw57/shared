...
def compile_reduce_kernel(source):
        kernel = ft.Kernel()
        kernel.compile_from_source(source)
        if not kernel.get_compile_status():
                print("Error compiling kernel:")
...
edge_points = engine.run(
        ...
        lena_surface.get_width(),        # how many horizontal samples
        lena_surface.get_height()        # how many vertical samples
        )
print('%i edge points found' % len(edge_points))
